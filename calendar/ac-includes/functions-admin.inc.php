...
function get_item($table,$id,$sql_condition=""){
	$sql="SELECT b.* FROM ".$table." AS b WHERE b.id='".mysql_real_escape_string($id)."' $sql_condition LIMIT 1";
...
function mod_item($table,$id_item,$values,$debug=false){
	...
	$update="UPDATE `".$table."` SET ".$mod_data." WHERE id='".mysql_real_escape_string($id_item)."' LIMIT 1";
...
function delete_item($table,$id,$debug=false){
	$del="DELETE FROM ".$table." WHERE id='".mysql_real_escape_string($id)."' LIMIT 1";
