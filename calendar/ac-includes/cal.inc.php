...
if(isset($_GET["id_item"])){
	if(!is_numeric($_GET["id_item"])){
	    ...
	    // check it exits and is active
		$sql="SELECT id FROM `".T_BOOKINGS_ITEMS."` WHERE `id`='".mysql_real_escape_string($_GET["id_item"])."' AND state=1 LIMIT 1";
		$res=mysql_query($sql) or die("Error checking items<br>".mysql_Error());
		if(mysql_num_rows($res)<1){
			die("There is no item with that reference");
		}else{
			define("ID_ITEM",$_GET["id_item"]);	#	id sent via  url, form session etc 
		}
     }	
} 	
else{
