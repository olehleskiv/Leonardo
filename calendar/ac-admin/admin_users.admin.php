...
if(isset($_POST["delete_it"])){
	...
	$sql="SELECT id FROM ".T_BOOKINGS_ITEMS." WHERE id_user=".mysql_real_escape_string($_POST["id"])."";
	$res=mysql_query($sql);
	while($row=mysql_fetch_assoc($res)){
		echo "<br>".$del="DELETE FROM ".T_BOOKINGS." WHERE id_item=".mysql_real_escape_string($row["id"])." LIMIT 1";
		mysql_query($del) or die("Error deelting item bookings");
	}
	//	delete calendar items
	$del="DELETE FROM ".T_BOOKINGS_ITEMS." WHERE id_user=".mysql_real_escape_string($_POST["id"])."";
